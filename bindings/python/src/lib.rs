...
use arco_core::model::{
    CscInput, PrettyBoundGroup, PrettyPrintAdapter, PrettyPrintOptions, PrettySection,
    format_ascii_number,
};
...
pub struct PyModel {
    ...
    array_print_specs: Vec<ArrayPrintSpec>,
}

const FLOAT_EQ_EPSILON: f64 = 1e-12;

struct ArrayPrintSpec {
    start_var_id: u32,
    len: usize,
    base_name: String,
    shape: Vec<usize>,
...
impl PyModel {
    ...
    fn find_array_print_spec(&self, var_id: VariableId) -> Option<&ArrayPrintSpec> {
    ...
    fn is_autogenerated_array_name(spec: &ArrayPrintSpec, var_id: VariableId, name: &str) -> bool {
...
impl PyModel {
    ...
    fn __str__(&self) -> String {
        let adapter = PythonPrettyAdapter { model: self };
        self.inner
...
fn typed_block(py: Python<'_>, func: Option<PyObject>, name: Option<String>) -> PyResult<PyObject> {
...
fn arco(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    let typed_block_fn = wrap_pyfunction!(typed_block, m)?;
